...
function StallOwnerDashboard() {
  ...
  async function fetchFlaggedReviews() {
    const token = getToken();
    try {
      const response = await fetch("http://localhost:3000/reviews/moderation/queue", {
  ...
  async function handleReviewAction(reviewId, action) {
    const confirmMsg = action === "delete" 
      ...
      : "Keep this review and remove the flag?";
      
    if (!window.confirm(confirmMsg)) return;

    const token = getToken();
    try {
      const response = await fetch(`http://localhost:3000/reviews/moderation/${reviewId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        toast.success(action === "delete" ? "Review deleted" : "Review approved");
        fetchFlaggedReviews(); // Refresh list
      }
    } catch (err) {
...
return (
    <div className="dashboard-page">
      <div className="dashboard-header">
        <h1>Stall Owner Dashboard</h1>
        <p>Manage your stalls and moderation</p>
      </div>

      {/* --- NEW: MAIN TABS --- */}
      <div className="filters" style={{ marginBottom: "24px", borderBottom: "1px solid #eee", paddingBottom: "16px" }}>
        <button
          onClick={() => setActiveTab("photos")}
          style={{
            backgroundColor: activeTab === "photos" ? "#121223" : "#f0f0f0",
        ...
        <button
          ...
          style={{
            backgroundColor: activeTab === "reviews" ? "#dc2626" : "#f0f0f0", // Red for moderation
            color: activeTab === "reviews" ? "white" : "#333",
          }}
        >
          Review Moderation ({flaggedReviews.length})
        </button>
      </div>

      {/* =========================================
          TAB 1: PHOTO SUGGESTIONS (Your Old Code)
         ========================================= */}
      {activeTab === "photos" && (
        <>
          {/* Stats Cards */}
      ...
      {/* =========================================
          TAB 2: REVIEW MODERATION (New Code)
         ========================================= */}
      {activeTab === "reviews" && (
        <div className="notifications-list">
          {flaggedReviews.length === 0 ? (
            <div className="empty-state">
              <p>No reviews flagged for moderation.</p>
            </div>
          ) : (
            flaggedReviews.map((review) => (
              <div key={review.reviewid} className="notification-card" style={{ borderLeft: "4px solid #dc2626" }}>
                <div className="notification-header">
                  <div>
                    <h3>{review.menuitems.name}</h3>
                    <p className="stall-name">
                      by {review.users?.name || "Anonymous"} • {review.rating} Stars
                    </p>
                  </div>
                ...
                <div className="notification-body">
                  <div style={{ padding: "16px", backgroundColor: "#f9fafb", borderRadius: "8px", margin: "12px 0", fontStyle: "italic", border: "1px solid #eee" }}>
                    "{review.reviewtext}"
                  </div>

                  <div className="notification-actions">
                    <button
                      className="btn-dismiss" // Red button style
                      onClick={() => handleReviewAction(review.reviewid, "delete")}
                      style={{ flex: 1 }}
                    >
                      Delete Review
                    </button>
                    <button
                      className="btn-approve" // Green button style
                      onClick={() => handleReviewAction(review.reviewid, "keep")}
                      style={{ flex: 1 }}
                    >
                      Keep Review (False Alarm)
                    </button>
                  </div>
                </div>
