...
// Removed the conflicting imports on line 4
...
// RENAMED from getUserById to getUser to avoid conflict and match app.js
async function getUser(req, res) {
  try {
    ...
    // Using userModel.getUserById explicitly
...
async function updateUser(req, res) {
  try {
    ...
    // Updated to use userModel.updateUserById
    const updated = await userModel.updateUserById(id, { Name, Email, Password: passwordHash, Role });
    res.json(updated);
  } catch (error) {
...
async function deleteUser(req, res) {
  try {
    const id = parseInt(req.params.id, 10);
    if (!id) return res.status(400).json({ error: 'Invalid id' });
    // Updated to use userModel.deleteUserById
